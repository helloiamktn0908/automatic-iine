...
def login():
	...
	#メアドと、パスワードを入力
	driver.find_element_by_name('username').send_keys('任意のメールアドレス')
	time.sleep(1)
	driver.find_element_by_name('password').send_keys('任意のパスワード')
	time.sleep(1)
...
def clicknice():
	target = driver.find_elements_by_class_name('_9AhH0')[25]
	...
	try:
		driver.find_elements_by_class_name('_9AhH0')[24].click()
		time.sleep(random.randint(2, 10))
		f = open('insta.txt','a')
		f.write("投稿をクリックしました\n")
		f.close()
...
if __name__ == '__main__':
	...
	taglist = ['洋服','ファッション', 'お洒落さんと繋がりたい', 'ハンドメイド', 'お洒落','小物']
